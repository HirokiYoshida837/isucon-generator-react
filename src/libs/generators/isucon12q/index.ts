...
/**
 * ISUCON12予選でのデータ生成処理のTypeScript移植。
 * <br/>
 * refs from : https://github.com/isucon/isucon12-qualify/blob/main/data/rand.go
 */

export function fakeTenantName(): string {
  return generateTenantPrefix() + generatePrefix2() + generateMiddle() + generateSuffix()
}


export function fakeCompetitionName(): string {
  const p = generateCompetitionPrefix()
  if (p == "") {
...
function generateTenantPrefix(): string {
  if (randomInt(10) < 2) {
    return "チーム"
  } else {
    return ""
...
function generateCompetitionPrefix(): string {

  const n = randomInt(40)
  if (n != 0 && n < 10) {
    return `第${n}回 `
  } else {
    return ""
...
function generateCompetitionSuffix(): string {

  const comps = ["杯", "カップ", "大会", `#${randomInt(10)}`, `vol.${randomInt(10)}`]
  return comps[randomInt(comps.length)]
}

function generatePrefix2(): string {
  if (randomInt(2) < 1) {
    return prefix2Data[randomInt(prefix2Data.length)]
  } else {
    return ""
  }
}

function generateMiddle(): string {
  return middleData[randomInt(middleData.length)]
}

function generateSuffix(): string {
  return suffixData[randomInt(suffixData.length)]
}

const prefix2Data = Array<string>(
